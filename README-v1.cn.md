...
* **状态定义**: `result` 消息支持 `ok` (成功), `fail` (失败), `timeout` (超时) 三种状态。
...
- [x] **协议实现**: 完成了 `Sandbox-Link` 消息结构的定义，增加了 `timeout` 状态支持。
- [x] **超时控制与定时器清理**:
  - 针对底层沙箱定时器泄露的 bug，实现了手动的超时管理逻辑。
  - 采用 `Promise.race` 结合 `try...finally` 结构，确保无论执行成功、超时或出错，定时器都能被可靠释放。
- [x] **Python 插件 (Beta)**:
...
4. **手动管理超时定时器**: 由于底层沙箱 `@runno/sandbox` 的内部定时器在执行完成后无法自动释放（导致 Node.js 进程挂起），决定在驱动层移除底层超时配置，改由驱动实现手动控制，并强制执行清理。

---
*记录日期: 2026年2月13日* (更新了超时控制与定时器清理实现)
