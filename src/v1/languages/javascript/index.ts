...
export class JavaScriptProvider implements LanguageProvider {
  ...
  async generate(
    ...
    if (!code.includes('export ') && !code.includes('module.exports')) {
      processedCode = code.replace(/function\s+(\w+)/g, 'export function $1');
      processedCode = processedCode.replace(/(const|let|var)\s+(\w+)\s*=\s*(async\s*)?\(/g, 'export $1 $2 = $3(');
    }

    const wrapper = await this.templateManager.getTemplate('javascript', 'universal_wrapper', '.js');

    const builder = new FSBuilder({ workdir });
    builder.addFiles({
      'user_code.js': processedCode,
      'main.js': wrapper
    });
