...
- [ ] **WASM 统一编译方案**: 探索在 Host 环境将不同语言的函数统一编译为 WASM，并在 WASM 运行时中执行。
  - **目的**: 利用 WASM 的跨平台特性和原生安全性，通过统一的二进制执行格式简化架构，并消除特定语言运行环境的依赖。
- [ ] **Host 原生隔离运行方案**: 探索在 Host 中直接编译运行代码，而非在虚拟机（VM）环境中运行。
  - **目的**: 追求极致的执行性能，通过轻量级的沙箱保护壳（如文件系统隔离、系统调用限制）在保证安全的同时，显著降低启动与运行开销。
